...
const modifiedRegulaFalsi = func => {
    ...
    for (let iteration = 0; iteration < maximum_iterations; iteration++) {
        ...
        let p = (x_min_local * func_of_x_max_local - x_max_local * func_of_x_min_local) / (func_of_x_max_local - func_of_x_min_local);
        let func_of_p = func(p);

        if (func_of_p === 0 || Math.abs(func_of_p) < tolerance)
            break;

        if (func_of_x_min_local * func_of_p > 0) {
            x_max_local = p;
            func_of_x_max_local = func_of_p / 2
        } else {
            x_min_local = p;
            func_of_x_min_local = func_of_p / 2
        }
    }
    console.log(steps)
