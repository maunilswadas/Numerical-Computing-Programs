...
const regulaFalsi = func => {
    ...
    for (let iteration = 0; iteration < maximum_iterations; iteration++) {
        ...
        let p = (x_min_local * func(x_max_local) - x_max_local * func(x_min_local)) / (func(x_max_local) - func(x_min_local));
        let func_of_p = func(p);

        steps.push({
            p1: [x_min_local, func(x_min_local)],
            p2: [x_max_local, func(x_max_local)],
            p_middle: [p, func_of_p]
        });

        if (func_of_p === 0 || Math.abs(func_of_p) < tolerance)
            break;

        if (func(x_min_local) * func_of_p > 0) {
            x_min_local = p;
        } else {
            x_max_local = p;
        }
